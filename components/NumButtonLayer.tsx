...
const NumButtonLayer = ({ show, onClick }: IProps) => {
  return (
    <div className={`
      fixed inset-x-0 ${show ? 'bottom-0' : '-bottom-100'}
      bg-primary backdrop-blur-[30px]
      shadow-md
      transition-all
      py-2.5 px-2
      flex flex-wrap
    ...
    `}>
      ...
      <button className="bg-light text-secondary mx-3 my-2.5 w-[calc(50%-24px)] py-3 rounded-lg" onClick={onClick}>
        清除
      </button>
    </div>
  )
