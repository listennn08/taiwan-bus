...
import BaseArrivalLabel from '@/components/BaseArrivalLabel'
...
const arriveDisplay = (stop?: IStop) => {
  if (stop?.TimeInfo?.StopStatus !== 0) {
    let text;
    switch (stop?.TimeInfo?.StopStatus) {
      case 1: 
        text = '尚未發車'
        break
      case 2:
...
const BusStatus = ({ route, routeInfo, shape }: IProps) => {
  ...
  const handleDirectionClick = (e: MouseEvent) => {
    ...
    if (element.dataset.value) {
      setDirection(parseInt(element.dataset.value))
    }
  }

  useEffect(() => {
    if (typeof window !== 'undefined') {
      if (!map) setMap(L.map('map-container', {
        zoomControl: false,
      }))
    }
  ...
  return (
    <section>
      <ul className="tabs">
        ...
        >
          <a href="#" onClick={handleDirectionClick} data-value={0} className="rounded-t-2xl">
            <IconCar className="text-secondary w-5 h-5 mr-2" />
      ...
      <div className="
        ...
        mb-12.5
      ">
        <ul className="max-h-127.5 overflow-y-scroll w-1/3 mr-5">
          {routeInfo[direction]?.Stops?.map((el) => (
            <li key={el.StopUID} className="flex items-center p-2.5">
              {arriveDisplay(el)}
              <div className="ml-4 text-lg">{el.StopName.Zh_tw}</div>
            </li>
          ))}
        </ul>
        <div id="map-container" className="w-2/3 h-128 rounded-2xl"></div>
      </div>
    </section>
  )
}
